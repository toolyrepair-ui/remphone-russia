...
Раздача: виртуальный хостинг REG.RU Host-0 (Apache за nginx, ISPmanager, IP `31.31.196.16`, корень `www/rem-phone.ru/`). DNS зона ns1/ns2.reg.ru: `A @` и `A www` → `31.31.196.16`. GitHub Pages не отключать (резерв). Выгрузка: `.github/workflows/deploy-reg-ru.yml` (`FTP_PATH`=`www/rem-phone.ru/`, обычный **FTP** — `ftps-legacy` с GitHub Actions даёт SSL wrong version number). HTTPS: Let’s Encrypt `rem-phone.ru_le1` (до 2026-11-15), имена `rem-phone.ru` и `www.rem-phone.ru`. HTTP→HTTPS в `.htaccess` только через `X-Forwarded-Proto =http` (не `%{HTTPS} off` — петля за nginx). www→apex на `https://`. Dotfiles и `.git` закрыты. Статика css/js/картинки/woff2 — кеш месяц; html/json без кеша. Заголовки: HSTS, CSP, nosniff, SAMEORIGIN, Referrer-Policy. Не публиковать на хостинг: `docs/`, `seo/` (внутренние данные), `.omniroute-src/`.

Дашборд: `/dashboard/` (noindex, HTTP Basic на хостинге: секреты `DASHBOARD_USER` / `DASHBOARD_PASSWORD`, файл `auth-config.php` не в git). Крон `dashboard.yml` — 08:30 и 16:30 Хабаровск: live `seo/health-check.mjs --live` → `dashboard/health.json`, сбор Метрики + опционально GA4/GSC → `dashboard/data.json`, коммит, FTP только папки `dashboard/` на REG.RU (тот же plain FTP). Push с `GITHUB_TOKEN` по-прежнему не запускает `deploy-reg-ru.yml`, поэтому FTP в том же джобе обязателен для живой страницы. Accepted Метрики хранится отдельно в `metrika_accepted`; блок `google` — сессии/заявки GA4 и `index.google_indexed` из GSC (см. `seo/GOOGLE_DASHBOARD.md`); защищённый `LEAD_STATS_URL` добавляет агрегаты доставки в `pipeline` (accepted/delivered/pending/failed, без PII), не подменяя Метрику. Клики звонка/WhatsApp/Telegram — отдельные цели, не конверсия. `seo-health.yml` — только CI на пуше HTML (локальный check, без коммита JSON).
...
очищается через 90 дней. `/health` публичный и минимальный (без pending/failed),
`/stats` защищён отдельным `LEAD_STATS_SECRET` и возвращает только агрегаты.
POST заявок принимает Origin rem-phone.ru (и localhost), режет тело > 8 КБ
и лимитирует частоту. Токен бота **не** хранится на сайте. Telegram ID владельца
в `config.js` не публиковать.
...
| `dashboard/` | Внутренний контроль (noindex, не в sitemap). Сбор: `node dashboard/collect.mjs`. Health: `dashboard/health.json`. Визиты — Метрика; блок `google` — GA4+GSC; `index.yandex_indexed` / `index.google_indexed` |
...
- Прайс на сайте — **статика** в HTML и в `price-calculator-data.js`. Локальный master: `data/full-phone-pricebook.*`; стадия 2 строится из `data/phone-models-2018-2026.*` + Moba-сбора `data/moba-all-models/moba-products.json` + средней арифметической наценки iPhone через `python scripts/build_mean_markup_master_pricebook.py`. Moba-сбор запускается `python scripts/scrape_moba_models.py` и имеет checkpoint для продолжения. Локальная прайс-программа: `tools/price-lookup.html`, данные для неё `tools/price-lookup-data.js`, генератор `python scripts/build_price_lookup_tool.py`. Публичный калькулятор: `price-calculator.html`, данные `price-calculator-data.js`, генератор `python scripts/build_site_price_calculator.py` (только итоговая цена, без Moba). Публичные «от» для брендов: `data/site-price-orientir.json` → `python scripts/apply_site_price_orientir.py`. Каталог моделей из `data/site-pricebook.json` → `python scripts/apply_repair_models.py` (чипы + `repair-models.js`, без live-fetch прайса). После правки master: `build_site_price_calculator.py`, затем деплой.
